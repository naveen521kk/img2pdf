...
def convertToPageSize(imgPath, height, width, border, bgColor):
    ...
    if height-2*(border)>mainImageHeight and width-2*(border)>mainImageWidth:
        #Image height is inside background height
        whiteBackground.paste(mainImage,box=(border,(height-2*(border)-mainImageHeight)//2))
    else:
        pass
...
print(convertToPageSize(r"C:\Users\Naveen User\Pictures\Capture.png",700,1300,100,"#000"))